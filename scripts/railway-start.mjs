...
// 10) ANTHROPIC_API_KEY FALLBACK (new): if neither OPENCLAW_AUTH_PROFILES_JSON nor OPENCLAW_AUTH_PROFILES_B64
//     is set, but ANTHROPIC_API_KEY is present, automatically generate and write auth-profiles.json
//     with the Anthropic provider key. This is the simplest way to configure the API key on Railway.
...
//   2. OPENCLAW_AUTH_PROFILES_B64  (base64 encoded JSON)
//   3. ANTHROPIC_API_KEY           (auto-generate auth-profiles.json for Anthropic provider)
...
console.log(”[railway-start] no auth profiles env vars found (OPENCLAW_AUTH_PROFILES_JSON, OPENCLAW_AUTH_PROFILES_B64, ANTHROPIC_API_KEY)”);
return;
}
...
// Priority 3: auto-generate from ANTHROPIC_API_KEY
if (anthropicKey) {
